...
procedure leer(var mae: arc_emp; var dato: emp);
begin
...
function eliminarEmpleado(var mae: arc_emp): boolean;
var
    dni, savePos: integer;

    exito, encontro : boolean;

    regm: emp;
begin
    
    writeln('Ingrese un DNI, se eliminará el empleado si existe');
    readln(dni);

    reset(mae);

    //Booleano inicializa en false
    exito := false;
    ...
    //Mientras no alcance el fin de archivo o no encuentre al empleado solicitado
    while((regm.dni <> valorAlto) and not(encontro)) do begin
        

        if(regm.dni = dni) then begin
            savePos := filepos(mae) - 1;
            encontro := true;
        end;
    ...
    //Si no alcanzo el fin de archivo es que encontro el empleado, guardar pos a reemplazar, exito es true
    if(encontro) then begin
        ...
        //Volver a la pos del reg a eliminar y sobreescribirlo
        seek(mae,savePos);
        write(mae,regm);

        exito := true;

        //Pararse al final del archivo para hacer el truncate
        seek(mae,filesize(mae) - 1);
        truncate(mae);
    end
    else
...
begin
    ...
    repeat
        ...
        writeln('7: Eliminar un empleado');
        writeln('0: Salir');
